...
const ToolTipsy = (function() {
  ...
  _tipsy_.init = function() {
    ...
    tipsy.forEach(el => {
      const config = new ToolTipsyConfig({
        id: `tipsy-${_tipsy_.tooltips.size}`,
  ...
  _tipsy_.attach = function(config) {
    if (config == null) {
      throw new Error(`ToolTipsy[attach]: not config has been provided`)
    }
    const _config = new ToolTipsyConfig({
      id: `tipsy-${_tipsy_.tooltips.size}`,
  ...
  function _buildTooltip(config) {
    ...
    if (target == null) {
      throw new Error(`ToolTipsy[_buildTooltip]: target element is not defined`)
    }
    ...
    _tipsy_.tooltips.set(config.id, config)
  }

  function _showTooltip() {
    const config = _tipsy_.tooltips.get(this.getAttribute("tipsy-id"))
    ...
    timeoutId = setTimeout(()=>{
      ...
      if(!Position[config.position]) {
        throw new Error(`ToolTipsy[_showTooltip]: Tooltip position '${config.position}' is invalid`)
      }
  ...
  function ToolTipsyConfig(config) {
    this.id = config.id
